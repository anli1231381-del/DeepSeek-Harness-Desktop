...
try {
  await app.dispatch('create_session', { session: { id: 'verify', projectId: null } });
  await app.dispatch('create_execution', { sessionId: 'verify', execution: { id: 'verify', status: 'queued', prompt: 'Create verification.txt in the current working directory containing exactly harness-openrouter-ok. Do not read other files or access the network. Then reply done.' } });
  await app.dispatch('run_execution', { executionId: 'verify' });
