...
export default function SessionSidebar({ onSelect }: { onSelect?: (sessionId: string) => void }) {
  ...
  useEffect(() => { setCurrent(getCurrentSession()); }, []);
  ...
  return <div className="session-sidebar">
    ...
    <button className="button" disabled={!desktop} onClick={() => {
      const id = crypto.randomUUID();
      void bridge('create_session', { session: { id, projectId: null, title: '新会话' } }).then(() => { setRevision(v => v + 1); handleSelect(id); }).catch(reason => setError(String(reason)));
    }}>新建会话</button>
    ...
    {projects.map(project => <div key={project.id} className="session-group"><div className="group-heading"><Folder size={14} /><strong>{project.name}</strong></div><div className="group-list">{getSessionsByProjectId(snapshot, project.id).length ? getSessionsByProjectId(snapshot, project.id).map(renderSession) : <div className="muted">(空)</div>}</div></div>)}
  </div>;
}
