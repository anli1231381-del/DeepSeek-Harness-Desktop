...
import { ArrowRight, Check, CheckCircle2, ChevronDown, ChevronRight, Circle, Clock3, FileCode2, FilePenLine, Folder, FolderOpen, FolderPlus, Home, Languages, LoaderCircle, MessageCircle, Monitor, Moon, Palette, Play, Plus, Puzzle, RefreshCw, Settings2, Square, Sun, TerminalSquare, Trash2, X, XCircle } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (data.sessions?.length) { selectSession(data.sessions[0].id); return; }
  ...
  return <div className={`app-shell mode-${workspaceMode}`}>
    <aside className="sidebar"><div className="brand"><span className="brand-mark"><img src="/app-icon.png" alt="" width="32" height="32" /></span><span>Harness <span className="brand-sub">桌面助手</span></span></div>{workspaceMode === 'chat' && <div className="workspace-switch" role="group" aria-label="切换模式"><button aria-label="工作模式" aria-pressed={false} onClick={() => setWorkspaceMode('work')}><Monitor size={17} /><span>返回工作</span></button><button aria-label="对话模式" aria-pressed={true}><MessageCircle size={17} /><span>官网对话</span></button></div>}
    ...
    <main className={`main-content ${page === '首页' ? 'conversation-home' : ''}`} hidden={workspaceMode !== 'work'}>
      ...
      {page === '首页' && (currentSessionId ? <SessionView key={currentSessionId} currentSessionId={currentSessionId} onOpenExtensions={() => setPage('扩展')} /> : <section className="session-view card"><div className="session-header"><h2>新对话 · 未关联项目</h2></div><div className="session-body"><p>{desktop ? '正在准备对话…' : '先沟通，再关联项目。浏览器仅预览界面，请打开桌面应用开始工作。'}</p></div></section>)}
