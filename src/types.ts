export type Page = '首页' | '项目' | '扩展' | '任务' | '修改' | '设置';
...
export type RuntimeStep = { id: string; label: string; status: 'ready' | 'pending' | 'optional' | 'failed'; detail: string };
export type Runtime = { busy?: boolean; available: boolean; connected: boolean; nodeVersion: string; nodePath: string; harnessPath: string; harnessVersion: string; source: 'local' | 'bundled' | 'missing'; message: string; validationError?: string; steps?: RuntimeStep[] };
export type Snapshot = { sessions?: import('./coreModels').Session[]; projects: Project[]; tasks: Task[]; connections: ModelConnection[]; settings: Settings; runtime: Runtime };
